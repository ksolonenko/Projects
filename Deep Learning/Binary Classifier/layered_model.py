...
def define_dense_model_single_layer(input_length, activation_f='sigmoid', output_length=1):
    ...
    model = keras.Sequential([
        layers.Dense(input_shape=(input_length,), units=output_length, activation=activation_f)
    ])
...
def define_dense_model_with_hidden_layer(input_length, 
    ...
    model = keras.Sequential([
        layers.Dense(input_shape=(input_length,), units=hidden_layer_size, activation=activation_func_array[0]),
...
def get_mnist_data():
    ...
    # flattening the 28x28 image, normalizing the data to the range [0,1] by min-max range and casting it to float32
